...
# Model state (shared across task calls within the same worker process)
_pipeline = None
...
def get_pipeline():
    ...
    if _pipeline is None:
        ...
        logger.info(f"Initializing Chronos model on {device}...")
...
def run_inference(telemetry_list_dict: List[Dict]) -> Dict:
    """
    Celery task that runs heavy ML inference.
    """
    id_column = "device_id"
    ...
    df = pd.DataFrame(telemetry_list_dict)
    ...
    # Resample to ensure fixed intervals
    ...
    # Simplified score calculation from predictions
    ...
    score_pred = max(0.0, max((_score_pred(item) for item in pred_readings), default=0.0))
    
    return {
        "anomaly_score": round(float(score_pred), 4),
