...
logger = logging.getLogger("ms3-ai-engine")
...
def _dispatch_alert(device_id: str, level: str, score: float) -> Optional[str]:
    ...
    payload = {
        ...
        "message": "Auto alert from AI engine (via RabbitMQ)",
    ...
    try:
        ...
        connection.close()
        logger.info(f"Alert {alert_id} published to RabbitMQ for device {device_id}")
        return alert_id
    except Exception as exc:
        logger.error(f"Failed to publish alert to RabbitMQ: {exc}")
        # Fallback to HTTP if RabbitMQ fails? Usually better to just log and let retry logic handle it
        return None
